...
public class UserController {
    ...
    // final 关键字意味着，一旦 userService 这个变量被初始化并指向一个 UserService 对象之后，就不能再让它指向别的
    // UserService 对象了

    // Spring 会自动查找并注入 UserService 的一个实例
    // @Autowired 的核心作用就是让 Spring 容器帮你管理对象之间的依赖关系，你只需要声明你需要什么类型的对象，Spring 就会负责把对应的
    // Bean “装配”进来
    @Autowired
    public UserController(UserService userService, TimerService timerService) {
    ...
    public ResponseEntity<?> logout(HttpServletRequest request) {
        ...
        if (session != null) {
            Duration duration = timerService.stop(); // 停止定时任务
            long durationSeconds = duration != null ? duration.getSeconds() : 0L;
            String durationText = duration != null
                    ? (duration.toMinutesPart() + "分" + duration.toSecondsPart() + "秒")
                    : "0秒";
            // 打印并将会话用时回传给前端，同时输出 sessionId 便于调试
            String sessionId = session.getId();
            System.out.println("总用时几分几秒: " + durationText + ", Session ID: " + sessionId);
            response.put("durationSeconds", durationSeconds);
            response.put("durationText", durationText);
            response.put("durationIso", duration != null ? duration.toString() : "PT0S");
            response.put("sessionId", sessionId);
            timerService.reset(); // 重置计时器
            String userAccount = (String) session.getAttribute("userAccount");
