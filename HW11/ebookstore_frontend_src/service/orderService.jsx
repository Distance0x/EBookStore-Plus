...
const OrderService = {  // 创建订单
  createOrder: async (shippingAddress, contactPhone) => {
    return await fetchApi(`/orders/create?shippingAddress=${shippingAddress}&contactPhone=${contactPhone}`, {
      method: 'POST'
    });
  },  // 获取用户所有订单（分页）
  getUserOrdersByPage: async (page = 0, size = 10, sortBy = 'createTime', sortDir = 'desc') => {
  ...
  // 用户搜索订单（分页）
  searchUserOrdersByPage: async (bookTitle, startTime, endTime, page = 0, size = 10, sortBy = 'createTime', sortDir = 'desc') => {
    const params = new URLSearchParams({
      page: page.toString(),
      size: size.toString(),
      sortBy,
  ...
  // 获取用户购买统计（分页）
  getUserPurchaseStatisticsByPage: async (startTime, endTime, page = 0, size = 10, sortBy = 'createTime', sortDir = 'desc') => {
    const params = new URLSearchParams({
      page: page.toString(),
      size: size.toString(),
      sortBy,
  ...
  getUserPurchaseStatisticsWithGlobal: async (startTime, endTime, page = 0, size = 10, sortBy = 'createTime', sortDir = 'desc') => {
    const params = new URLSearchParams({
      page: page.toString(),
      size: size.toString(),
      sortBy,
