...
public class OrderController {
    ...
    public ResponseEntity<?> createOrderFromCartKafka(
            @RequestParam String shippingAddress,
            @RequestParam String contactPhone,
            HttpServletRequest request){
        try {
            ...
            User user = (User) session.getAttribute("user");
            // 用UserId作为Key，组装kafka
            String message = user.getId() + "," + shippingAddress + "," + contactPhone;
            // 发送到kafka
            kafkaTemplate.send("order_submit", user.getId().toString(), message);
            response.put("message", "Order is being processed");
