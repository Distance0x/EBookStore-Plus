        ...
        websocket.onmessage = (event) =>{
            ...
            try {
              ...
              if (msg.type === 'ORDER_RESULT') {
                const text = msg.message || '订单操作完成'
                
                if (msg.status === 'SUCCESS') {
                  console.log('订单创建成功:', text)
                  const payload = { type: 'success', text, ts: msg.timestamp || Date.now(), ack: false }
                  ...
                  console.log('订单创建失败:', text)
                  const payload = { type: 'failed', text, ts: msg.timestamp || Date.now(), ack: false }
                  setOrderResult(payload)
                  sessionStorage.setItem('latestOrderResult', JSON.stringify(payload))
                  
                } else {
                  // 其他状态（INFO / WARNING 等）
                  console.log('订单消息:', msg.status, text)
                //   messageAPI.info(text)
                }
              }
            } catch (e) {
              console.error('解析 WebSocket 消息失败:', e)
              // 如果不是 JSON 格式，尝试按旧格式处理（兼容性处理）
