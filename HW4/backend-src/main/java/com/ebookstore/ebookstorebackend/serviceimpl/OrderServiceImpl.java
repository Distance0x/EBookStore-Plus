...
public class OrderServiceImpl implements OrderService {    @Autowired
    ...
    public OrderDTO createOrderFromCart(Long userId, String shippingAddress, String contactPhone) {
        ...
        for (Cart cartItem : cartItems) {
            ...
            // 从 BookStock 检查库存是否充足
            BookStock bookStock = bookStockDao.findByBookId(book.getId())
                    .orElseThrow(() -> new RuntimeException("图书《" + book.getTitle() + "》库存信息不存在"));
            
            if (bookStock.getStock() < cartItem.getQuantity()) {
                throw new RuntimeException("图书《" + book.getTitle() + "》库存不足，当前库存：" + bookStock.getStock() + "，需要：" + cartItem.getQuantity());
            }
        ...
        // 扣减库存（在创建订单时就扣减，使用原子操作）
        for (Cart cartItem : cartItems) {
            Book book = cartItem.getBook();
            boolean success = bookStockDao.decrementStock(book.getId(), cartItem.getQuantity());
            if (!success) {
                throw new RuntimeException("图书《" + book.getTitle() + "》库存扣减失败，可能库存已不足");
            }
        }
          // 更新订单总金额
    ...
    public void deleteOrder(Long orderId) {
        ...
        if ("待支付".equals(order.getStatus()) || "PENDING".equals(order.getStatus())) {
            for (OrderItem orderItem : order.getOrderItems()) {
                // 恢复库存（原子增加）
                bookStockDao.incrementStock(orderItem.getBookId(), orderItem.getQuantity());
            }
        }
