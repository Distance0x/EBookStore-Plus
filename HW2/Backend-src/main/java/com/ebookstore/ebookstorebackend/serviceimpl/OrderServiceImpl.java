...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderDTO createOrderFromCart(Long userId, String shippingAddress, String contactPhone) {
        ...
        logger.info("获取用户成功: {}", user.getAccount());
        // 获取用户购物车
        ...
        if (!deletedBookTitles.isEmpty()) {
            throw new RuntimeException(
                    "购物车中包含已下架的图书：" + String.join("、", deletedBookTitles) + "。请先清理购物车中的已下架图书后再下单。");
        }
        // 创建订单
        ...
        for (Cart cartItem : cartItems) {
            ...
            if (book.getStock() < cartItem.getQuantity()) {
                throw new RuntimeException("图书《" + book.getTitle() + "》库存不足，当前库存：" + book.getStock() + "，需要："
...
//        int result = 10 / 0;
        ...
        logger.info("订单保存成功，总金额: {}", totalAmount);
        // 创建订单项并保存
        ...
        for (Cart cartItem : cartItems) {
            ...
            logger.info("保存订单项 - 图书: {}, 数量: {}, 单价: {}",
                    book.getTitle(), cartItem.getQuantity(), book.getPrice());
        }
//        int result = 10 / 0;
    ...
    @Override
    @Transactional(readOnly = true)
    public List<OrderDTO> searchUserOrders(Long userId, String bookTitle, LocalDateTime startTime,
    ...
    @Override
    @Transactional(readOnly = true)
    public PurchaseStatisticsDTO getUserPurchaseStatistics(Long userId, LocalDateTime startTime,
            LocalDateTime endTime) {
        // 获取用户在指定时间范围内的已完成订单
        ...
        for (Order order : completedOrders) {
            ...
            for (OrderItem item : order.getOrderItems()) {
                ...
                if (bookStatsMap.containsKey(bookKey)) {
                    ...
                    existingStats.setTotalPrice(existingStats.getTotalPrice()
        ...
        try {
            ...
            for (Order order : completedOrders) {
                ...
                if (userConsumptionMap.containsKey(userId)) {
                    ...
                    UserConsumptionStatisticsDTO.UserConsumptionDTO userConsumption = new UserConsumptionStatisticsDTO.UserConsumptionDTO(
                            userId,
                            user.getAccount(),
                            user.getName(),
                            user.getEmail(),
            ...
            List<UserConsumptionStatisticsDTO.UserConsumptionDTO> userConsumptions = new ArrayList<>(
                    userConsumptionMap.values());
            userConsumptions.sort((a, b) -> b.getTotalSpent().compareTo(a.getTotalSpent()));
            return new UserConsumptionStatisticsDTO(
                    userConsumptions,
    ...
    @Override
    public Page<OrderDTO> searchUserOrdersByPage(Long userId, String bookTitle, LocalDateTime startTime,
            LocalDateTime endTime, Pageable pageable) {
        try {
            Page<Order> orderPage;

            if (bookTitle != null && !bookTitle.trim().isEmpty() && startTime != null && endTime != null) {
                orderPage = orderDao.findUserOrdersByBookTitleAndTimeRange(userId, bookTitle, startTime, endTime,
                        pageable);
            } else if (bookTitle != null && !bookTitle.trim().isEmpty()) {
    ...
    @Override
    public Page<PurchaseStatisticsDTO> getUserPurchaseStatisticsByPage(Long userId, LocalDateTime startTime,
            LocalDateTime endTime, Pageable pageable) {
        try {
            // 获取用户已完成订单
            ...
            List<PurchaseStatisticsDTO.BookStatisticsDTO> pageContent = start >= allBookStats.size() ? new ArrayList<>()
                    : allBookStats.subList(start, end);
    ...
    @Override
    public Page<OrderDTO> searchAllOrdersByPage(String bookTitle, LocalDateTime startTime, LocalDateTime endTime,
            Pageable pageable) {
        try {
            Page<Order> orderPage;
    ...
    @Override
    public Page<BookSalesStatisticsDTO> getBookSalesStatisticsByPage(LocalDateTime startTime, LocalDateTime endTime,
            Pageable pageable) {
        try {
            ...
            List<BookSalesStatisticsDTO.BookSalesDTO> pageContent = start >= allBookSales.size() ? new ArrayList<>()
                    : allBookSales.subList(start, end);
    ...
    @Override
    public Page<UserConsumptionStatisticsDTO> getUserConsumptionStatisticsByPage(LocalDateTime startTime,
            LocalDateTime endTime, Pageable pageable) {
        try {
            // 获取已完成订单
            List<Order> allOrders = orderDao.findCompletedOrdersByTimeRange(startTime, endTime);
            ...
            // 将Map转换为List并排序
            List<UserConsumptionStatisticsDTO.UserConsumptionDTO> allUserConsumptions = new ArrayList<>(
                    userConsumptionMap.values());
            ...
            List<UserConsumptionStatisticsDTO.UserConsumptionDTO> pageContent = start >= allUserConsumptions.size()
                    ? new ArrayList<>()
                    : allUserConsumptions.subList(start, end);
    ...
    @Override
    public StatisticsPageResponseDTO<PurchaseStatisticsDTO> getUserPurchaseStatisticsWithGlobal(Long userId,
    ...
    @Override
    public StatisticsPageResponseDTO<BookSalesStatisticsDTO> getBookSalesStatisticsWithGlobal(LocalDateTime startTime,
            LocalDateTime endTime, Pageable pageable) {
        try {
            // 获取分页数据
    ...
    @Override
    public StatisticsPageResponseDTO<UserConsumptionStatisticsDTO> getUserConsumptionStatisticsWithGlobal(
            LocalDateTime startTime, LocalDateTime endTime, Pageable pageable) {
        try {
            // 获取分页数据
